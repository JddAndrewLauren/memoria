...
omission: no locking, no queue, no reconciliation, no merge, and no
multi-file atomicity. A write applies to the file the client read, whole, or
...
def create(
    ...
    ``write`` deliberately cannot do this: it gates on a staleness token
    minted from a file that already exists, so a path with no file behind it
    never matches one. That was right while every durable file was already
    there and the only question was whether it had moved underneath a reader.

    Promotion (#17) is the first act that *creates* durable state - a
    candidate becomes an entry, and the entry file did not exist a moment
    ago - so the rule needs a second door rather than a way around it. It is
    here, in the module that owns writes, so that a promotion is still
    confined to a durable state class, still replaced atomically, and still
    committed and attributed to whoever acted. The alternative - letting the
    extraction write the file itself - would make a promotion an
    unattributed, uncommitted act, which is exactly what the write path
    exists to prevent.

    An existing file is ``Rejected(outcome="exists")`` rather than
    overwritten. That is the same shape as a stale token and for the same
    reason: it is a normal outcome the caller handles, not an exception. It
    also makes promotion safely repeatable - running the pass twice creates
    nothing new instead of flattening an entry the author has since edited.

    Parent directories are created, because a subject the author added has a
    directory and a subject seeded by ``write_builtin_subjects`` has one, but
    a brand new subject may not.
    """
    path = _confined(repository, PurePosixPath(relative_path))
    if path.exists():
        return Rejected(outcome="exists", path=relative_path)
    if not actor.human:
        checkpoint(repository)
