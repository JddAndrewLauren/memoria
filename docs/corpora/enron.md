...
Each custodian archive holds `.eml` under `native_*`, extracted plain text
under `text_*`, one XML load file, and the attachments as loose native files. By
the archive's layout the text and XML are the same messages again (assumed, not
verified by `Message-ID`). **Only the `.eml` are raw units**, so the fetch
unpacks nothing else: `manifest.sync` numbers every file under `raw/**` and an
ID once given is never reused (ADR-0006). The sidecars stay in the zip under
`.archives/`.
...
```bash
# 1. Fetch the four slice custodians (the `slice:` list in the yaml). Verifies
#    sha1 against the pins, records sha256 on first fetch, then enforces it.
#    Idempotent. Drop the --custodian flags to fetch the whole measured pool.
.venv/bin/python scripts/fetch-enron.py --dest ../enron-evidence \
    --custodian scholtes-d --custodian semperger-c \
    --custodian crandall-s --custodian salisbury-h
...
The `slice:` list in the yaml is documentation; nothing reads it. Each
custodian lands in its own directory, so selecting custodians is the whole slice
rule and no dedup step exists. The numbers below were measured over the full
twelve-custodian pool, not the slice alone.
...
The full numbers live in `RECON.md` beside the corpus, regenerated by
`scripts/enron-recon.py`; the figures quoted below are transcribed from the
2026-09-01 run over the twelve-custodian pool. Four findings are load-bearing
enough to record here, because they are inputs to the email converter (#78)
rather than facts about one slice.

**1. Two in five messages (39.6% of the pool, 23% to 75% by custodian) do not
...
Deleting that one line before parsing is the entire fix and leaves 9 of 10,490
messages with a parse defect.
...
is present on **8 of 10,490** messages and `References` on a handful. Enron ran
...
This retires two of the plan's verification steps as written. Step 4's "if the
`In-Reply-To` reply graph is too low, re-pick the custodian set" branch does not
help: no custodian in this export clears an `In-Reply-To` threshold, so the set
was chosen on cross-custodian correspondence (below) and `Thread-Index` parent
presence instead. Step 6's "each fixture parses with stdlib" is inverted on
purpose: finding 1 is that the export does not, so the fixtures carry the defect
and the test asserts it fails raw and parses after the one-line repair. The
fixtures are invented rather than hand-scrubbed copies, so the scrub grep in the
test is a guard against a real message being pasted in later, not a confirmation.
