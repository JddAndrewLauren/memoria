...
| `thread_id` | Email only. The thread the message belongs to, for grouping in the index and the viewer. A thread is never a record. Added 2026-09-01. |
...
| `in_reply_to` | Email only. The `SRC-` ID of the message this one replies to, resolved from `Message-ID` / `In-Reply-To` within the same export; empty when the headers are missing or the parent is not in the archive. Added 2026-09-01. |
