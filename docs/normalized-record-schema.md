...
| `thread_id` | Email only. The thread the message belongs to, for grouping in the index and the viewer. A thread is never a record. Resolved from `Message-ID` when present; otherwise, when `Thread-Index` is present, the first 22 bytes of its base64 decoding (docs/corpora/enron.md finding 2). Added 2026-09-01. |
